...
class CRF(nn.Module):
    ...
    Args:
        ...
        eos_tag_id (int): integer representing the end of sentence symbol in your target.
    ...
    def init_weights(self):
        ...
        # enforce constraints (row=from column=to) with a big negative number
    ...
    def log_likelihood(self, emissions, tags, mask=None):
        ...
        Args:
            ...
            tags (torch.Tensor): Sequence of (correct) labels.
        ...
        if mask is None:
            mask = torch.ones(emissions.shape[:2], dtype=torch.float) # (batch_size, seq_len)

        scores = self._compute_scores(emissions, tags, mask=mask)
        partition = self._compute_log_partition(emissions, mask=mask)
        return torch.sum(scores - partition)

    def _compute_scores(self, emission, tags, mask):
        ...
        first_tags = tags[:, 0] # take the first words from each batch
        last_valid_idx = mask.int().sum(1) - 1 # get the list of length of seq in each batch
    ...
    def _compute_log_partition(self, emissions, mask):
        ...
        for i in range(1, seq_length):
            ...
            for tag in range(nb_labels):
                ...
                # broadcast the transition scores to all the batches
                # (bs, nb_labels)
            ...
            # change the current values of alpha by new ones if we didn’t reach a pad position
            # keeping the same values if we reach a pad position.
